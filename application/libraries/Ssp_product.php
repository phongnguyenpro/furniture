...
//              if($columns[$j]['db']=="hinhdaidien")
//                                      $row[ $column['dt'] ] = "<img width='50' height='60' src='".URL."public/upload/images/thumb_hinhsanpham/". $data[$i][ $columns[$j]['db'] ]."' >";
//				else	
    ...
    static function simple($request, $conn, $table, $primaryKey, $columns) {
     ...
     if($order=='')
        ...
        if ($request['category_id'] == -1) {
            ...
            $resTotalLength = self::sql_exec($db, "SELECT COUNT(*)
			 FROM product
                         "
            );
            ...
            $recordsFiltered = $resFilterLength[0][0];
            // Total data set length
            $resTotalLength = self::sql_exec($db, "SELECT count(DISTINCT(productcategory_detail.product_id))
			 FROM product,productcategory_detail
                         where  product.product_id=productcategory_detail.product_id  and  ( $sqlwhere ) "
            );
