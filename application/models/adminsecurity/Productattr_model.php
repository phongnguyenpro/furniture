...
//            }
    ...
    function delete($productattr_id, $type = 1)
    {
        //delete product details
        $kq = $this->mydb->select("SELECT product_detail_id FROM productattr_detail WHERE productattr_id=:productattr_id and product_detail_id!=-1 and product_detail_id!=-2 ", array("productattr_id" => $productattr_id));
        if (!empty($kq)) {
            foreach ($kq as $value) {
                $temp[] = $value['product_detail_id'];
            }
            $sqlwhere = "product_detail_id=" . implode(" or product_detail_id=", $temp);
            $this->mydb->exec("DELETE FROM product_detail WHERE $sqlwhere");
        }

        //delete attr value
        $this->mydb->deleteall("attr_val", "productattr_id=:productattr_id", array("productattr_id" => $productattr_id));
        //delete product attr detail
        $this->mydb->deleteall("productattr_detail", "productattr_id=:productattr_id", array("productattr_id" => $productattr_id));
