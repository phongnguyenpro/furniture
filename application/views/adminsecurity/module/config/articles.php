...
<form action="<?= ADMIN_URL . "module/update" ?>" method="POST">
    ...
    <div class="form-group uk-form-row">
        ...
        <div class="col-sm-8">
            <input min="1" value="<?= isset($config["module_articles_limit"] )?$config["module_articles_limit"]:"" ?>" required class="form-control" name="module_articles_limit"  type="number" placeholder="">
        </div>
    </div>
    <div class="form-group uk-form-row">
        <label class="control-label col-sm-3" for="email">Loại sản phẩm:</label>
        <div class="col-sm-8">
            <select name="module_articles_type">
                <?php
                foreach ( get_articles_type() as $k => $v) {
                    ?>
                    <option  <?= (isset($config["module_articles_type"])&&$config["module_articles_type"]==$k)?"selected":"" ?> value="<?= $k ?>"><?= $v ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
