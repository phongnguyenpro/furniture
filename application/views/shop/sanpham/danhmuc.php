                    ...
                    <div class="block_content">
                        ...
                        <div class="layered layered-filter-price">
                            ...
                            <?php foreach ($this->data['boloc'] as $key=>$value){ 
                                     switch($value['thongtin']['tenthuoctinhchon']) { 
                                         case "Màu sắc": {?>
                            ...
                            <div class="layered-content filter-color">
                                 <div class="check-box-list listgiatri" data-label="<?= $key ?>">
                                     ...
                                     <li>
                                        <input <?= in_array($value2['id_giatrithuoctinhchon'],$this->data['filter'])==true?"checked":"" ?> class="filter"  value="<?= $value2['id_giatrithuoctinhchon']?>" type="checkbox" id="<?= $value2['id_giatrithuoctinhchon']?>" name="filter[]">
...
foreach ($this->sapxep as $key=>$value){
                           ?>
                            <option <?= $value['selected'] ?> value="<?= $key ?>"><?= $value['ten']?></option>
                            <?php } ?>
                        </select>
                        <div class="sort-product-icon sortby btn btn-default">
                            <i  data-type="asc" class="fa fa-sort-alpha-asc <?= $this->data['sortby']=="asc"?"sortbyactive":"" ?>" ></i>
                            <i data-type="desc" class="fa fa-sort-alpha-desc <?= $this->data['sortby']=="desc"?"sortbyactive":"" ?>"></i>
                        </div>
                                         </div></div>
                            </div> 
                       
                        <div class="text-center">  <button type="button" class="btn btn-primary btnlocsanpham">TÌM NGAY</button></div>
                            </form>
                        </div>
                    </div>
                </div>
         ...
         <!-- Center colunm-->
                ...
                <div class="subcategories">
                    <ul>
                        ...
                        <li  data-tuychon="tatca" class="tuychonsanpham <?= $this->data['tuychonsanpham']=='tatca'?"current-categorie":"" ?>"   >
                         <a >Tất cả</a>
                 </li>
                         <li data-tuychon="noibat" class="tuychonsanpham <?= $this->data['tuychonsanpham']=='noibat'?"current-categorie":"" ?>" >
                             <a >Sản nổi bật</a>
                        </li>
                        <li data-tuychon="giamgia" class="tuychonsanpham <?= $this->data['tuychonsanpham']=='giamgia'?"current-categorie":"" ?>">
            ...
            <div id="view-product-list" class="view-product-list">
                    <h2 class="page-heading">
                        <span class="page-heading-title"><?= $this->data['thongtindanhmuc']['ten'] ?></span>
                    </h2>
